...
SELECT facturas.*, empleados.Apellido, empleados.Nombre FROM facturas
INNER JOIN empleados ON facturas.EmpleadoID = empleados.empleadoID;
...
SELECT Clientes.contacto, SUM(PrecioUnitario * Cantidad) as Total FROM facturas
INNER JOIN facturadetalle ON facturas.facturaID = facturadetalle.facturaId
INNER JOIN clientes ON facturas.ClienteID = clientes.ClienteID
GROUP By contacto order by SUM(PrecioUnitario * Cantidad) desc LIMIT 5;

-- 8. Generar un listado de facturas, con los campos id, nombre y apellido del cliente,
-- fecha de factura, y país de envío, ordenado de manera descendente por fecha de factura y limitado a 10 filas.
SELECT f.FacturaID, c.contacto, f.FechaFactura, f.PaisEnvio, SUM(PrecioUnitario* Cantidad) as Total
FROM facturas AS f
INNER JOIN facturadetalle ON f.facturaID = facturadetalle.facturaId
INNER JOIN clientes c ON f.ClienteID = c.ClienteID
GROUP By f.FacturaID order by f.FechaFactura desc LIMIT 10;